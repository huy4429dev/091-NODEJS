...
controller.index = (req, res) => {
   ...
   `;
    ...
    const sqlRatioOrderRoom = `select count(*) / (select count(*) 
                            from rooms r 
                            where r.userid = ${userId}) ratio, 
                                g.day from (
                                select  roomId ,  date_format(createTime, "%d-%m-%Y") day
                                from orders o 
                                where o.userId = ${userId} and o.status != 3 and (createTime >= '${startOfMonth}' and createTime <= '${endOfMonth}')
                                group by roomId ,  date_format(createTime, "%d-%m-%Y")
                            ) as g
                            group by g.day`;
